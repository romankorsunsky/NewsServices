...
public class ArticlesInfoController {
    ...
    public ResponseEntity<List<ArticleDTO>> getArticlesByDateLatest() {   
        List<ArticleDTO> articles = serv.findArticlesSortedByDateLatest();
        if(articles.isEmpty()){
    ...
    @GetMapping("/{id}")
    public ResponseEntity<ArticleDTO> getArticlesById(@PathVariable("id") Long id) {
        Optional<ArticleDTO> article = serv.findArticleById(id);
    ...
    public ResponseEntity<ArticleDTO> addArticle(@RequestBody Article article) {
        ...
        try{
            ArticleDTO articleSaved = serv.saveArticle(article);
    ...
    public ResponseEntity<ArticleDTO> deleteArticleById(@PathVariable Long id){
        Optional<ArticleDTO> deletedArticle = serv.findArticleById(id);
